        ...
        <div class="space-y-3 pb-3">
            <div class="hero-bg w-full bg-contain object-fill bg-no-repeat border-none relative">
                <div class="w-full h-full md:h-auto flex flex-col justify-start items-center space-y-2 pt-12 md:pt-10">
                    <span
                        class="text-[11.333vw] md:text-[6vw] lg:text-[4vw] text-center text-white font-medium lato-light">
                    ...
                    <div class="flex justify-center items-center w-screen md:w-[400px]">
                        <div
                            ...
                            <span role="img" aria-label="bell" class="anticon anticon-bell text-[#818488]">
                                <svg viewBox="64 64 896 896" focusable="false" data-icon="bell" width="1em"
                ...
                <div class="mt-[40px] md:mt-[180px]">
                    <div class="flex flex-col justify-center items-center w-screen md:w-full">
                        <div class="w-[93vw] md:w-[400px] bg-white rounded-lg shadow relative flex flex-col justify-start items-center px-3 pb-3"
                            wire:ignore>
                            ...
                            <div class="ant-slider w-full mt-8" wire:ignore>
                                <input type="range" id="loan-slider"
                                    ...
                                    min="{{ $activeLoanPackage ? data_get($activeLoanPackage->config_loans, 'min_amount', 2000000) : 2000000 }}"
                                    max="{{ $activeLoanPackage ? data_get($activeLoanPackage->config_loans, 'max_amount', 20000000) : 20000000 }}"
                                    step="100000" value="{{ $amount }}" onchange="updateAmount(this)">
                            ...
                            <div class="w-full flex flex-row justify-between items-center mt-4">
                                @if ($activeLoanPackage)
                                    @foreach ($quickAmounts as $index => $quickAmount)
                                        <div
                                            class="text-base text-[var(--primary)] {{ $index === count($quickAmounts) - 1 ? 'font-semibold' : '' }}">
                                    ...
                                    @if ($activeLoanPackage)
                                        ...
                                        @if (is_array($termMonths) && !empty($termMonths))
                                            @foreach ($termMonths as $months)
                                                <button onclick="selectTerm({{ $months }})"
                                        ...
                                        @else
                                            <button
                                                class="text-sm px-5 py-1 rounded-2xl bg-yellow-400 text-black cursor-pointer">
                                                7 ngày
                                            </button>
                                        @endif
                                    @else
                                        <button onclick="selectTerm(6)"
                        ...
                        <button onclick="submitWithAmount()"
...
<script>
    function updateAmount(slider) {
        ...
        document.getElementById('loan-slider').setAttribute('data-amount', amount);
    }
    ...
    function selectTerm(months) {
        ...
        buttons.forEach(function(button) {
            ...
            if (buttonMonths === months) {
                button.className =
                    'text-sm px-5 py-1 rounded-2xl cursor-pointer term-button bg-yellow-400 text-black';
            } else {
                button.className =
                    'text-sm px-5 py-1 rounded-2xl cursor-pointer term-button text-gray-400 border border-solid border-gray-500 border-opacity-25';
            }
        });
